...
{commentbegin1} {BEGIN COMMENT1; fprintf(yyout,"Begin a comment:\n");}
<COMMENT1>{commentelement1} {BEGIN INITIAL;fprintf(yyout,"%s this comment End!\n",yytext);}
{white}+ ;
...
{id} {int i=0;int flag=0;for(;i<=count;i++){if(strcmp(idlist[i],yytext)==0){flag=1;break;}}if(flag==0) {count++;i=count;strcpy(idlist[i],yytext);}fprintf(yyout,"ID %d %s %d\n",ID,yytext,i);}
